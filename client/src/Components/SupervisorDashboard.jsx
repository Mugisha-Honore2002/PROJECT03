...
export default function SupervisorDashboard() {
  ...
  const [teacherSearch, setTeacherSearch] = useState(null);
  const [courseSearch, setCourseSearch] = useState(null);
  const [enrollSearch, setEnrollSearch] = useState(null);
  ...
  const handleUnassignTeacher = async (groupId) => {
    ...
    try {
      const response = await api.delete(`/api/assignment/unassign/${groupId}`);
  
      setSuccess(response?.data?.message || "Teacher unassigned.");
      fetchAll();
    } catch (err) {
  ...
  const filteredTeachers = teacherSearch ? [teacherSearch] : teachers;
  const filteredCourses = courseSearch  ? [courseSearch] : courses;
  const filteredEnrollments = enrollSearch ? enrollments.filter(e => e.student_name?.toLowerCase().includes((enrollSearch.student_name||"").toLowerCase())) : enrollments;
      ...
      <main className="flex-1 p-8 overflow-y-auto">
        ...
        {page === "students" && (
          <div>
            ...
            <p className="mb-4 text-sm text-slate-400">All registered students.</p>
            <div className="mb-6"><SearchBar items={students} searchKeys={["full_name","email"]} placeholder="Search students…" onSelect={setStudentSearch} renderLabel={s => s.full_name} renderSub={s => s.email} renderAvatar={s => s.full_name?.[0]?.toUpperCase()} accentColor="text-violet-600" /></div>
        ...
        {page === "teachers" && (
          <div>
            ...
            <div className="mb-6"><SearchBar items={teachers} searchKeys={["full_name","email"]} placeholder="Search teacher…" onSelect={setTeacherSearch} renderLabel={t => t.full_name} renderSub={t => t.email} renderAvatar={t => t.full_name?.[0]?.toUpperCase()} accentColor="text-violet-600" /></div>
            {loading ? <p className="text-sm text-slate-400">Loading…</p> : (
              <div className="overflow-hidden bg-white border rounded-2xl border-slate-100">
                <table className="w-full text-sm">
                  <thead><tr className="border-b border-slate-100">{["ID","Name","Email","Joined",""].map(h => <th key={h} className="px-5 py-3 text-xs font-semibold tracking-widest text-left uppercase text-slate-400">{h}</th>)}</tr></thead>
        ...
        {page === "courses" && (
          <div>
            <div className="flex items-center justify-between mb-6">
              <div>
                <h1 className="mb-1 text-2xl font-bold text-slate-900">Courses & Groups</h1>
                <p className="mb-4 text-sm text-slate-400">Manage all courses and their groups.</p>
                <SearchBar items={courses} searchKeys={["name","code"]} placeholder="Search courses…" onSelect={setCourseSearch} renderLabel={c => c.name} renderSub={c => c.code} accentColor="text-violet-600" />
              </div>
        ...
        {page === "enrollments" && (
          <div>
            ...
            <p className="mb-4 text-sm text-slate-400">All student enrollments ({enrollments.length} total).</p>
            <div className="mb-6"><SearchBar items={enrollments} searchKeys={["student_name","course_name"]} placeholder="Search by student or course…" onSelect={e => setEnrollSearch(e)} renderLabel={e => e.student_name} renderSub={e => e.course_name} renderAvatar={e => e.student_name?.[0]?.toUpperCase()} accentColor="text-violet-600" /></div>
            {loading ? <p className="text-sm text-slate-400">Loading…</p> : (
              <div className="overflow-hidden bg-white border rounded-2xl border-slate-100">
                <table className="w-full text-sm">
